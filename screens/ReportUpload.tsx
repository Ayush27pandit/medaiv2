...
import { analyzeHealthReportText } from '../services/api';
...
const ReportUpload = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const result = await analyzeHealthReportText(extractedText);
      console.log('AI result:', result);
      router.push({ pathname: '/ReportResults', params: { result: JSON.stringify(result), image }});
    } catch (err) {
      console.error('OCR/Upload error:', err);
      Alert.alert('Error', 'Failed to analyze report image. Please try again.');
    } finally {
      setLoading(false);
    }
  };
