...
const ReportResults = () => {
  ...
  try {
    ...
    image = params.image || '';
    console.log('AI Report Result:', content);
  } catch (err) {
  ...
  let fixedContent = content
    .replace(/^##\s*Report Title:/gm, '## Report Title:')
    .replace(/^##\s*🧑 Patient Name \(if available\):/gm, '## Patient Name (if available):')
    .replace(/^##\s*Summary\/Advice:/gm, '## Summary/Advice:')
    .replace(/^##\s*Key Findings:/gm, '## Key Findings:')
    .replace(/^##\s*Test Results:/gm, '## Test Results:')
    .replace(/^##\s*Doctor's Notes \(if available\):/gm, "## Doctor's Notes (if available):");
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Card style={styles.resultCard}>
        <Card.Content>
          ...
          <Markdown style={markdownStyles}>{fixedContent && fixedContent.trim().length > 0 ? fixedContent : 'No analysis result found. Please try a clearer image.'}</Markdown>
        </Card.Content>
      </Card>
